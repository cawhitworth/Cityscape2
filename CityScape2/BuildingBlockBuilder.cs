...
namespace CityScape2
{
    class BuildingBlockBuilder
    {
        ...
        public IGeometry Build(Vector3 c1, int xStories, int yStories, int zStories)
        {
            ...
            var front = new Panel(c1, new Vector2(c2.X - c1.X, c2.Y - c1.Y), Panel.Plane.XY, Panel.Facing.Out, m_StoryCalc.ToTexture(tx1), m_StoryCalc.ToTexture(tx2));

            tx1 = m_StoryCalc.RandomPosition();
            tx2 = new Vector2(tx1.X + xStories, tx1.Y + yStories);

            var back = new Panel(c2, new Vector2(c1.X - c2.X, c1.Y - c2.Y), Panel.Plane.XY, Panel.Facing.In, m_StoryCalc.ToTexture(tx1), m_StoryCalc.ToTexture(tx2));
